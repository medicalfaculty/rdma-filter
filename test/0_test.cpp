...
int main(int argc, char **argv) {
    ...
    std::vector<uint64_t> to_insert(REAL_INSERT_COUNT), to_lookup(LOOKUP_COUNT_MAX), lookup_temp = {};
    int lookup_count = 0;
    ...
    std::cout << "=== DramBF Wormhole Experiment ===" << std::endl;
    struct bloomfilter dram_bf;
    bloomfilter_init(&dram_bf, INSERT_COUNT, FALSE_POSITIVE_RATE);

    // debug
    for (auto i : to_insert) {
        bloomfilter_insert(&dram_bf, i);
    }
    for (auto i : to_lookup) {
        if (!bloomfilter_lookup(&dram_bf, i)) {
            false_positive_count++;
        }
    }
    std::cout << "FPR: " << 1.0 * false_positive_count / lookup_count << std::endl;
    bloomfilter_destroy(&dram_bf);
    exit(0);
    ...
    for (int i = 0; i < 20; i++) {
        ...
        for (int j = 0; j < SINGLE_ROUND_COUNT; j++) {
            bloomfilter_insert(&dram_bf, to_insert[i * SINGLE_ROUND_COUNT + j]);
        }
        ...
        start_time = std::chrono::high_resolution_clock::now();
        for (int j = 0; j < SINGLE_ROUND_COUNT; j++) {
            if (bloomfilter_lookup(&dram_bf, to_insert[i * SINGLE_ROUND_COUNT + j])) {
                true_positive_count++;
            }
        }
        end_time = std::chrono::high_resolution_clock::now();
        ...
        for (int j = 0; j < lookup_count; j++) {
            if (!bloomfilter_lookup(&dram_bf, to_lookup[j])) {
                true_negative_count++;
            } else {
                false_positive_count++;
            }
