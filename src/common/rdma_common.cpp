...
int rdma_one_side(ibv_qp *qp, int wr_id, ibv_sge *sge, uint64_t remote_addr, uint32_t rkey, ibv_wr_opcode opcode) {
    ...
    // std::cerr << "[DEBUG] rdma_one_side: wr_id=" << wr_id << ", opcode=" << opcode_str  << ", remote_addr=0x" << std::hex << remote_addr << std::dec  << ", rkey=" << rkey << std::endl;
...
int rdma_atomic_cas(ibv_qp *qp, int wr_id, ibv_sge *sge, ibv_cq *cq, uint64_t remote_addr, uint32_t rkey, uint64_t compare_add, uint64_t swap) {
    ...
    *((uint64_t *)(sge->addr)) = compare_add + 1;
    ...
    // 自旋直到获取锁
    int retry = 0;
    // std::cerr << "[DEBUG] rdma_atomic_cas: wr_id=" << wr_id << ", remote_addr=0x" << std::hex << remote_addr  << std::dec << ", rkey=" << rkey << ", compare_add=" << compare_add << ", swap=" << swap << std::endl;
    while (retry++ < COUNT_RETRY_CAS_MAX) {
        ...
        if (retry % 100 == 0) {
            // std::cerr << "[DEBUG] rdma_atomic_cas: waiting for lock, wr_id=" << wr_id << ", retry=" << retry << ", result=" << result << ", expected=" << compare_add << std::endl;
        }
        usleep(200 + rand() % 300);
    }
