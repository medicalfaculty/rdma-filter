...
Status RdmaCF_Cli_insert_impl(struct RdmaCF_Cli *cli, uint32_t bucket_index, uint16_t tag) {
    ...
    for (int count = 0; count < cli->k_max_kick; count++) {
        ...
        //    std::cerr << "[DEBUG] RdmaCF_Cli_insert_impl: cuckoo kick count=" << count   << ", index_curr=" << index_curr << ", tag_curr=" << tag_curr << std::endl;
        }
...
int RdmaCF_Cli_lock_bucket(struct RdmaCF_Cli *cli, uint32_t index_bucket, int index_buf_bucket) {
    ...
    // std::cerr << "[DEBUG] RdmaCF_Cli_lock_bucket: index=" << index_bucket << ", mutex_addr=0x"  << std::hex << addr_mutex << std::dec << ", buf_index=" << index_buf_bucket << std::endl;
